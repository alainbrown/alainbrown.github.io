...
In May 2024, Andrej Karpathy published [llm.c](https://github.com/karpathy/llm.c) — a GPT-2 training implementation in ~4,000 lines of C and CUDA. No Python. No PyTorch. Just raw GPU kernels training a 124-million-parameter language model on 10 billion tokens in 45 minutes on 8 H100 GPUs.
...
Karpathy's original llm.c trained GPT-2 (124M) in 90 minutes on 8 A100 GPUs for about $20 on Lambda Labs. The modded-nanogpt community did it in under 90 seconds on 8 H100s.

The 31x speedup didn't come from a PhD thesis. It came from the same techniques you'd use to optimize a database.

## What the model is actually doing

A language model predicts the next word. Given "the cat sat on the", it assigns probabilities to every possible next token. Training adjusts the model's internal numbers — its [parameters](#parameters) — until those predictions become good. GPT-2 has 124.5 million of them.
...
3. **Backward pass** — compute [gradients](#gradients) for all 124 million parameters (how much did each one contribute to the error?) and nudge them in the direction that reduces it
...
**The architecture** is a [transformer](#transformer) — a stack of 12 identical layers, each containing two operations:
...
Each layer's output is added back to its input via a [residual connection](#residual-connection) — a skip wire that gives information a direct highway through the network. Without these, deep networks are nearly impossible to train. The gradient signal would decay to nothing before reaching the early layers.
...
| Target | ≤3.28 cross-entropy loss on FineWeb validation set |
...
**Flash Attention 3** — Attention involves computing a matrix of scores (every token against every other token), applying softmax, then multiplying by values. Naively, this requires materializing the full n×n attention matrix in GPU HBM (high bandwidth memory — big but slow). [Flash Attention](https://arxiv.org/abs/2205.14135) fuses these operations into a single kernel that tiles the computation into blocks that fit in SRAM (small but fast), never materializing the full matrix.
