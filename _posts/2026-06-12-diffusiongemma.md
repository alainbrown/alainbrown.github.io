...
An autoregressive model factorizes text as a product of conditionals: `P(token | everything to the left)`. It samples position 1, appends it, samples position 2, and so on. The output grows one token at a time, and once a token is out you can't revise it.
...
The visualizer wraps the model in a Gradio app and renders every denoising step as a frame. Both backends hand back the intermediate canvas at each step, so what you're seeing is the model's actual state mid-denoise, not a replay. The color coding is the key to reading it:
...
**NVIDIA, via `transformers`.** The app drives the model's `TextDiffusionStreamer`, which hands back the full draft canvas at every denoising step and the committed tokens once a block locks in. It defaults to 4-bit quantization (~13 GB), which fits a 24 GB card; you can drop to 8-bit on a 48 GB card, or run full precision (~55 GB) on an A100/H100 for the best quality. It runs under Docker with one command.

**Apple silicon, via MLX.** No NVIDIA GPU needed. On an Apple-silicon Mac you can run the real model natively with the pre-quantized 4-bit checkpoint [`mlx-community/diffusiongemma-26B-A4B-it-4bit`](https://huggingface.co/mlx-community/diffusiongemma-26B-A4B-it-4bit) (~16.6 GB to download, and it fits a 24 GB Mac). This path uses `mlx-vlm`'s `stream_generate` with `diffusion_show_unmasking=True`, which surfaces the same per-step canvas. This is the backend I recorded the videos on, locally on a Mac. It runs on the host rather than in Docker, since Docker Desktop on macOS has no Metal GPU access.
